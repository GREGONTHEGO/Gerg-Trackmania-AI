...
class TrackmaniaDataset(Dataset):
    def __init__(self, states):
        ...
        self.move = np.stack([state['move'] for state in states], axis=0).astype(np.int64) # used for mu
        # self.turn = np.stack([state['turn'] for state in states], axis=0).astype(np.int64) # used for std
...
def compute_reward(prev_state, current_state, move, tim, alpha=10.0):
...
def lidar_clearance_bonus(delta_spd, move):# update this to know left from right
    if delta_spd < 0.0:
        return -1.0
    center_idk = 9
    bonus = 0.0
    for i in range(len(frame_buffer)):
        ...
        # mean = np.mean(forward_distance, forward_left, forward_right)
